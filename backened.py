...
def super_nexus_analyze_text(text: str, user: dict) -> dict:
    """
...
def super_nexus_fraud_analysis(item: Any, user: dict) -> dict:
    ...
    text_report = super_nexus_analyze_text(f"{item.title} {item.description}")
...
def update_profile(profile: UserProfile, user: dict = Depends(get_current_user)):
    ...
    conn.execute("UPDATE users SET name = ?, phone = ?, profile_pic = ?, address = ?, is_admin = ? WHERE id = ?", 
                 (name, profile.phone, profile.profile_pic, profile.address, is_admin, user['id']))
    conn.commit()
    conn.close()
...
def delete_listing(item_id: int, user: dict = Depends(get_current_user)):
    ...
    if item['seller_id'] != user['id'] and user['is_admin'] < 2:
        conn.close()
        raise HTTPException(status_code=403, detail="Not authorized. Only Primary Admin can force remove.")
...
def star_listing(item_id: int, req: StarRequest, user: dict = Depends(get_current_user)):
    conn = get_db_connection()
    listing = conn.execute("SELECT stars_count FROM listings WHERE id = ?", (item_id,)).fetchone()
    if not listing:
        conn.close()
    ...
    requested_add = max(1, req.count)
    allowed_add = min(requested_add, 20 - current_count)
    ...
    if vote:
        conn.execute("UPDATE stars_votes SET count = count + ? WHERE user_id = ? AND listing_id = ?", (allowed_add, user['id'], item_id))
    else:
        conn.execute("INSERT INTO stars_votes (user_id, listing_id, count) VALUES (?, ?, ?)", (user['id'], item_id, allowed_add))
        
    conn.execute("UPDATE listings SET stars_count = stars_count + ? WHERE id = ?", (allowed_add, item_id))
    conn.commit()
    conn.close()
    return {"status": "starred", "added": allowed_add, "new_total": listing['stars_count'] + allowed_add}
